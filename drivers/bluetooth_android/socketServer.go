...
func (b *BluetoothAdapter) GetBluetoothServerSocket(env uintptr) (*BluetoothServerSocket, error) {
	var errMsgC *C.char
	sock := C.createBluetoothServer(C.uintptr_t(env), b.self, &errMsgC)
	if errMsgC != nil {
		err := errors.New(C.GoString(errMsgC))
		C.free(unsafe.Pointer(errMsgC))
		return nil, err
	}
	socket := &BluetoothServerSocket{self: sock}
	socket.FetchAddress(env)
	return socket, nil
}

// FetchAddress it is usefully if GetAddress return empty string, it try to set internal address
func (b *BluetoothServerSocket) FetchAddress(env uintptr) {
	var errMsgC *C.char
